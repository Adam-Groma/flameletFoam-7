...
#include "flameletExternalCoupledTemperatureMixedFvPatchScalarField.H"
...
void Foam::flameletExternalCoupledTemperatureMixedFvPatchScalarField::writeHeader
(
    OFstream& os
) const
{
...
Foam::flameletExternalCoupledTemperatureMixedFvPatchScalarField::
flameletExternalCoupledTemperatureMixedFvPatchScalarField
(
    const fvPatch& p,
...
flameletExternalCoupledTemperatureMixedFvPatchScalarField
(
    const flameletExternalCoupledTemperatureMixedFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
flameletExternalCoupledTemperatureMixedFvPatchScalarField
(
    const flameletExternalCoupledTemperatureMixedFvPatchScalarField& ecmpf
)
:
...
void Foam::flameletExternalCoupledTemperatureMixedFvPatchScalarField::transferData
(
    OFstream& os
) const
{
    ...
    typedef compressible::flameletTurbulenceModel cmpTurbModelType;
    ...
    static word thermoName(flameletBasicThermo::dictName);
    ...
    if (db().foundObject<cmpTurbModelType>(turbName))
    {
        ...
        const flameletBasicThermo& thermo = turbModel.transport();

        const fvPatchScalarField& hep = thermo.he().boundaryField()[patchi];

        qDot = turbModel.alphaEff(patchi)*hep.snGrad();
    }
    else if (db().foundObject<flameletBasicThermo>(thermoName))
    {
        const flameletBasicThermo& thermo = db().lookupObject<flameletBasicThermo>(thermoName);
...
void Foam::flameletExternalCoupledTemperatureMixedFvPatchScalarField::evaluate
(
...
void Foam::flameletExternalCoupledTemperatureMixedFvPatchScalarField::write
(
    Ostream& os
) const
{
