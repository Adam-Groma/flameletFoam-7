...
#include "flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField.H"
...
flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField::
flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField
(
    const fvPatch& p,
...
flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField
(
    const flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField& psf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
:
    ...
    flameletTemperatureCoupledBase(patch(), psf.KMethod(), psf.kappaName()),
...
void flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField::updateCoeffs()
{
    if (updated())
    {
        return;
    ...
    typedef flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField thisType;
    ...
    {
        // Swap to obtain full local values of neighbour K*delta
        KDeltaNbr.ref() = nbrField.kappa(nbrField)*nbrPatch.deltaCoeffs();
    }
    else
    {
        KDeltaNbr.ref() = contactRes_;
    }
    mpp.distribute(KDeltaNbr.ref());
...
void flameletTurbulentTemperatureRadCoupledMixedFvPatchScalarField::write
(
    Ostream& os
) const
{
