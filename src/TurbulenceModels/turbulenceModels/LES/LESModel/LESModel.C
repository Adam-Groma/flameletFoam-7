...
:
    ...
    transportVarZ_(LESDict_.lookupOrDefault<Switch>("transportVarZ", false)),
    ...
    varZ_(this->mesh().template lookupObjectRef<rhoReactionThermo>("thermo").varZ()),
    Chi_(this->mesh().template lookupObjectRef<rhoReactionThermo>("thermo").Chi())
{
...
template<class BasicTurbulenceModel>
void Foam::LESModel<BasicTurbulenceModel>::correctVarZ()
{
	if (transportVarZ_)
	{
	    ...
	    (
	        (
	            ...
	            - 2.0*DZEff()*magSqr(fvc::grad(this->mesh().template lookupObjectRef<rhoReactionThermo>("thermo").Z()))
	            + 2.0*this->rho_*Chi_
	        )
	    );

	    varZEqn.ref().relax();
	    varZEqn.ref().boundaryManipulate(varZ_.boundaryFieldRef());
	...
	{
	    varZ_ = CvarZ_ * sqr(delta()) * magSqr(fvc::grad(this->mesh().template lookupObjectRef<rhoReactionThermo>("thermo").Z()));
	    varZ_.correctBoundaryConditions();
	}
...
template<class BasicTurbulenceModel>
void Foam::LESModel<BasicTurbulenceModel>::correctChi()
{
	if (transportVarZ_)
	{
	    Chi_ = this->mu()/(Sc_*this->rho_)*magSqr(fvc::grad(this->mesh().template lookupObjectRef<rhoReactionThermo>("thermo").Z())) + Cchi_*DZt()*varZ_/(2.0*this->rho_*sqr(delta()));
	    Chi_.correctBoundaryConditions();
	}
	else
	{
	    Chi_ = Cchi_ * DZEff()/this->rho_ * magSqr(fvc::grad(this->mesh().template lookupObjectRef<rhoReactionThermo>("thermo").Z()));
	    Chi_.correctBoundaryConditions();
	}
