...
namespace Foam
{
    defineTypeNameAndDebug(flameletLESdelta, 0);
    defineRunTimeSelectionTable(flameletLESdelta, dictionary);
}

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::flameletLESdelta::flameletLESdelta
(
    const word& name,
...
Foam::autoPtr<Foam::flameletLESdelta> Foam::flameletLESdelta::New
(
    const word& name,
    const flameletTurbulenceModel& turbulence,
...
)
{
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown flameletLESdelta type "
            << deltaType << nl << nl
            << "Valid flameletLESdelta types are :" << endl
            << dictionaryConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<flameletLESdelta>(cstrIter()(name, turbulence, dict));
}
    ...
    else
    {
        ...
        if (cstrIter == dictionaryConstructorTablePtr_->end())
        {
            FatalErrorInFunction
                ...
                << dictionaryConstructorTablePtr_->sortedToc()
                << exit(FatalError);
            return autoPtr<flameletLESdelta>();
        }
        else
        {
            return autoPtr<flameletLESdelta>(cstrIter()(name, turbulence, dict));
