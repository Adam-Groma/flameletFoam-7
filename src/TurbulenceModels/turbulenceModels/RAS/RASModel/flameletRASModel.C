...
#include "flameletRASModel.H"

// * * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * //

template<class BasicTurbulenceModel>
void Foam::flameletRASModel<BasicTurbulenceModel>::printCoeffs(const word& type)
{
    if (printCoeffs_)
    {
...
template<class BasicTurbulenceModel>
Foam::flameletRASModel<BasicTurbulenceModel>::flameletRASModel
(
    const word& type,
...
template<class BasicTurbulenceModel>
Foam::autoPtr<Foam::flameletRASModel<BasicTurbulenceModel>>
Foam::flameletRASModel<BasicTurbulenceModel>::New
(
    const alphaField& alpha,
    const rhoField& rho,
    const volVectorField& U,
...
)
{
    ...
    (
        ...
        ).subDict("RAS").lookup("flameletRASModel")
    );
    ...
    if (cstrIter == dictionaryConstructorTablePtr_->end())
    {
        FatalErrorInFunction
            << "Unknown flameletRASModel type "
            << modelType << nl << nl
            << "Valid flameletRASModel types:" << endl
            << dictionaryConstructorTablePtr_->sortedToc()
            << exit(FatalError);
    }

    return autoPtr<flameletRASModel>
    (
...
template<class BasicTurbulenceModel>
bool Foam::flameletRASModel<BasicTurbulenceModel>::read()
{
...
template<class BasicTurbulenceModel>
void Foam::flameletRASModel<BasicTurbulenceModel>::correct()
{
    BasicTurbulenceModel::correct();
}

template<class BasicTurbulenceModel>
void Foam::flameletRASModel<BasicTurbulenceModel>::correctVarZ()
{
...
template<class BasicTurbulenceModel>
void Foam::flameletRASModel<BasicTurbulenceModel>::correctChi()
{
    Chi_ = Cchi_ * this->epsilon()/this->k() * varZ_;
    bound(Chi_, 0.0);
