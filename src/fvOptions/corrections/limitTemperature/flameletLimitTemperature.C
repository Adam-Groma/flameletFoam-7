...
#include "flameletLimitTemperature.H"
#include "fvMesh.H"
...
namespace fv
{
    defineTypeNameAndDebug(flameletLimitTemperature, 0);
    addToRunTimeSelectionTable
    (
        option,
        flameletLimitTemperature,
        dictionary
    );
}
}
...
Foam::fv::flameletLimitTemperature::flameletLimitTemperature
(
    const word& name,
    ...
    phase_(coeffs_.lookupOrDefault<word>("phase", word::null))
{
    ...
    const flameletBasicThermo& thermo =
        mesh_.lookupObject<flameletBasicThermo>
        (
            IOobject::groupName(flameletBasicThermo::dictName, phase_)
        );
...
bool Foam::fv::flameletLimitTemperature::read(const dictionary& dict)
{
...
void Foam::fv::flameletLimitTemperature::correct(volScalarField& he)
{
    const flameletBasicThermo& thermo =
        mesh_.lookupObject<flameletBasicThermo>
        (
