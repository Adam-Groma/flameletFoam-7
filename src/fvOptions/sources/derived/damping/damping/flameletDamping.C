...
namespace fv
{
    defineTypeNameAndDebug(flameletDamping, 0);
}
}
...
Foam::tmp<Foam::volScalarField::Internal> Foam::fv::flameletDamping::forceCoeff() const
{
...
Foam::fv::flameletDamping::flameletDamping
(
    const word& name,
...
bool Foam::fv::flameletDamping::read(const dictionary& dict)
{
    if (flameletCellSetOption::read(dict))
    {
        ...
        if (!foundAll && foundAny)
        {
            WarningInFunction
                ...
                << "the flameletDamping. The flameletDamping will be applied uniformly across "
