...
#include "flameletRotorDiskSource.H"
...
namespace Foam
{
    namespace fv
    {
        defineTypeNameAndDebug(flameletRotorDiskSource, 0);
        addToRunTimeSelectionTable(option, flameletRotorDiskSource, dictionary);
    }

    template<> const char* NamedEnum<fv::flameletRotorDiskSource::geometryModeType, 2>::
        names[] =
    {
        "auto",
        "specified"
    ...
    const NamedEnum<fv::flameletRotorDiskSource::geometryModeType, 2>
        fv::flameletRotorDiskSource::geometryModeTypeNames_;

    template<> const char* NamedEnum<fv::flameletRotorDiskSource::inletFlowType, 3>::
        names[] =
    {
        "fixed",
        "surfaceNormal",
    ...
    const NamedEnum<fv::flameletRotorDiskSource::inletFlowType, 3>
        fv::flameletRotorDiskSource::inletFlowTypeNames_;
}


// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

void Foam::fv::flameletRotorDiskSource::checkData()
{
    // Set inflow type
...
void Foam::fv::flameletRotorDiskSource::setFaceArea(vector& axis, const bool correct)
{
    area_ = 0.0;
...
void Foam::fv::flameletRotorDiskSource::createCoordinateSystem()
{
...
void Foam::fv::flameletRotorDiskSource::constructGeometry()
{
    const vectorField& C = mesh_.C();
...
Foam::tmp<Foam::vectorField> Foam::fv::flameletRotorDiskSource::inflowVelocity
(
...
Foam::fv::flameletRotorDiskSource::flameletRotorDiskSource
(
    const word& name,
...
:
    ...
    cylindrical_(),
    rMax_(0.0),
    trim_(flameletTrimModel::New(*this, coeffs_)),
...
void Foam::fv::flameletRotorDiskSource::addSup
(
    fvMatrix<vector>& eqn,
    const label fieldi
)
...
bool Foam::fv::flameletRotorDiskSource::read(const dictionary& dict)
{
