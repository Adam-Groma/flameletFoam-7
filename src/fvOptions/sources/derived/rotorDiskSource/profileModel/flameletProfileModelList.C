...
#include "flameletProfileModelList.H"

// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

Foam::flameletProfileModelList::flameletProfileModelList
(
    const dictionary& dict,
    const bool readFields
)
:
    PtrList<flameletProfileModel>(),
    dict_(dict)
{
    if (readFields)
    {
        ...
        if (modelNames.size() > 0)
        {
            ...
            forAll(modelNames, i)
            {
                ...
                (
                    i,
                    flameletProfileModel::New(dict.subDict(modelName))
                );
            }
        }
        else
...
void Foam::flameletProfileModelList::connectBlades
(
...
) const
{
    ...
    forAll(names, bI)
    {
        ...
        forAll(*this, pI)
        {
            const flameletProfileModel& pm = this->operator[](pI);
        ...
        if (index == -1)
        {
            ...
            forAll(*this, i)
            {
                const flameletProfileModel& pm = this->operator[](i);
