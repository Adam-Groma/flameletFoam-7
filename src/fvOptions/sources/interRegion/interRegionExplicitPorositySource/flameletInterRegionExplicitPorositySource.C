...
#include "flameletInterRegionExplicitPorositySource.H"
#include "fvMesh.H"
...
namespace fv
{
    defineTypeNameAndDebug(flameletInterRegionExplicitPorositySource, 0);
    addToRunTimeSelectionTable
    (
        option,
        flameletInterRegionExplicitPorositySource,
        dictionary
    );
}
}
...
void Foam::fv::flameletInterRegionExplicitPorositySource::initialise()
{
    if (!firstIter_)
    {
        return;
...
Foam::fv::flameletInterRegionExplicitPorositySource::flameletInterRegionExplicitPorositySource
(
    const word& name,
...
:
    flameletInterRegionOption(name, modelType, dict, mesh),
...
void Foam::fv::flameletInterRegionExplicitPorositySource::addSup
(
    fvMatrix<vector>& eqn,
    const label fieldi
)
...
bool Foam::fv::flameletInterRegionExplicitPorositySource::read(const dictionary& dict)
{
    if (flameletInterRegionOption::read(dict))
    {
