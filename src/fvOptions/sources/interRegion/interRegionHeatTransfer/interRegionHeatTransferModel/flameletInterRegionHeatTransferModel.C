...
#include "flameletInterRegionHeatTransferModel.H"
...
namespace fv
{
    defineTypeNameAndDebug(flameletInterRegionHeatTransferModel, 0);
}
}
...
void Foam::fv::flameletInterRegionHeatTransferModel::setNbrModel()
{
    if (!firstIter_)
    {
        return;
    ...
    forAll(fvOptions, i)
    {
        ...
        {
            nbrModel_ = &const_cast<flameletInterRegionHeatTransferModel&>
            (
                refCast<const flameletInterRegionHeatTransferModel>(fvOptions[i])
            );
            nbrModelFound = true;
            break;
        }
...
void Foam::fv::flameletInterRegionHeatTransferModel::correct()
{
    if (master_)
    {
...
Foam::fv::flameletInterRegionHeatTransferModel::flameletInterRegionHeatTransferModel
(
    const word& name,
...
void Foam::fv::flameletInterRegionHeatTransferModel::addSup
(
    fvMatrix<scalar>& eqn,
    const label fieldi
)
{
    ...
    if (semiImplicit_)
    {
        ...
        {
            if (mesh_.foundObject<flameletBasicThermo>(flameletBasicThermo::dictName))
            {
            ...
            else
            {
                FatalErrorInFunction
                    << " on mesh " << mesh_.name()
                    << " could not find object flameletBasicThermo."
